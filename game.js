...
let level='normal',running=false,score=0,hits=0,misses=0,combo=0,bestCombo=0,timeLeft=120000;
...
function spawnBuddy(){const open=$$('.hole:not(.up)');const current=$$('.hole.up').length;if(!open.length||current>=settings[level].max)return;const h=open[Math.floor(Math.random()*open.length)],who=Math.random()<.5?'charan':'yesh';const img=new Image();img.className='buddy keyed';img.src=`assets/${who}-key.png`;removeGreen(img);h.innerHTML='';h.appendChild(img);h.classList.add('up');h.dataset.hit='0';h.onclick=()=>hitBuddy(h,who);const duration=settings[level].up*(activePower==='slow'?1.7:1);h._timer=setTimeout(()=>{if(h.dataset.hit==='0'){combo=0;misses++;updateHud()}clearHole(h)},duration)}
